...
PROCESSOR_TARGET := $(BUILD_DIR)/processor.out
PROCESSOR_SRC_DIR := processor
PROCESSOR_SRC := $(PROCESSOR_SRC_DIR)/processor.c
PROCESSOR_SRC_ALL := $(PROCESSOR_SRC) $(PROCESSOR_SRC_DIR)/main.c
PROCESSOR_OBJS := $(PROCESSOR_SRC:%.c=$(BUILD_DIR)/%.o)
PROCESSOR_OBJS_ALL := $(PROCESSOR_SRC_ALL:%.c=$(BUILD_DIR)/%.o)
...
$(PROCESSOR_TARGET): $(PROCESSOR_OBJS_ALL)
	$(CC) $(CFLAGS) $(LDFLAGS) $^ -o $@

$(FUN_TESTS_TARGET): $(FUN_TESTS_OBJS) $(COMPILER_OBJS) $(PROCESSOR_OBJS)
	$(CC) $(CFLAGS) $(LDFLAGS) $^ -o $@
